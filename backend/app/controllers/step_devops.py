...
def trigger_ci():
    ...
    result, piplineID, piplineUrl, success = triggerPipeline(
        requirementID, branch, serviceInfo, ciConfigList[0]
    )
    if not success:
...
def plugin_ci():
    ...
    piplineJobs, docker_mage, success = getPipelineStatus(
        pipeline_id, repopath, ciConfigList[0]
    )
...
def trigger_cd():
    ...
    if len(image) < 1:
        raise Exception(
            _(
                "1Could not get deployment docker image, Please “Trigger continuous integration” first."
            )
        )
